...
def clean(text):
    ...
    )  # Remove lone numbers like page numbers
    return text
...
def filter(sequences):
    ...
    for seq in sequences:
        if (
            len(seq.split()) > 10 and seq not in seen
        ):  # Minimum length and not duplicate
            filtered.append(seq)
            seen.add(seq)
    return filtered
...
def normalize(sequences):
    ...
    for seq in sequences:
        # seq = seq.lower()
        seq = re.sub(
            r"\b{A-Za-z0-9._%+-]+\.[A-Z|a-z]{2,}\b", "[EMAIL]", seq
        )  # Anonymize emails
...
def nomnom(pdf_dir=None):
    ...
    console.print(
        f"[bold green]Saved training data from {len(files)} PDFs to training_data.txt[/bold green]"
    )
